"""
Стресс-тест API polza.ai
Запуск: python test_api_polza.py
...
# ===== НАСТРОЙКИ =====
API_KEY = "<POLZA_AI_API_KEY>"  # <-- ВСТАВЬ СВОЙ КЛЮЧ СЮДА
...
PAYLOAD_STREAM = {
    "model": MODEL,
    "messages": [
        {"role": "user", "content": "Привет, как дела? Ответь коротко."},
    ],
    "max_tokens": 50,
    "temperature": 0.5,
    "stream": True,
...
async def single_request(session: aiohttp.ClientSession, request_id: int, timeout_s: float = 10.0, stream: bool = True) -> dict:
    ...
    payload = PAYLOAD_STREAM if stream else PAYLOAD_SYNC
    t0 = time.time()
        ...
        async with session.post(URL, json=payload, headers=HEADERS, timeout=aiohttp.ClientTimeout(total=timeout_s)) as resp:
            ...
            if stream:
                ...
                if choices:
                    ...
                    result["tokens"] = body.get("usage", {}).get("completion_tokens", 0)
...
async def test_sync_first(timeout: float = 15.0):
    """Тест 0: один синхронный запрос (без стрима) — проверка что API вообще работает."""
    print(f"\n{BOLD}{CYAN}═══ ТЕСТ 0: Один запрос БЕЗ стрима (проверка API) ═══{RESET}")

    connector = aiohttp.TCPConnector(force_close=True)
    async with aiohttp.ClientSession(connector=connector) as session:
        r = await single_request(session, 1, timeout_s=timeout, stream=False)
        print_result(r)
    return [r]
...
async def main():
    ...
    if API_KEY == "<POLZA_AI_API_KEY>":
        print(f"\n{RED}  ОШИБКА: Вставь свой API ключ в переменную API_KEY в начале файла!{RESET}")
        return
    ...
    # Тесты
    await test_sync_first(timeout=15.0)
