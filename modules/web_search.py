...
async def search(query: str, max_results: int = 5) -> Optional[str]:
    ...
    text = "\n".join(lines)
    log.info(f"[SEARCH] DDG: {len(results)} results ({elapsed:.0f}ms) for: {search_query}")
    return text
...
def needs_search(text: str) -> Optional[str]:
    ...
    Триггерит ТОЛЬКО на прямые команды поиска и конкретные вопросы,
    требующие актуальной информации из интернета.
    """
    text_lower = text.lower()
    
    # Слишком короткие фразы — точно не поиск
    if len(text_lower.strip()) < 10:
        return None
    
    # === 1. Прямые команды поиска ===
    direct_triggers = [
        "загугли", "найди в интернете", "поищи в интернете",
        "погугли", "посмотри в интернете",
    ...
    # === 2. Конкретные вопросы, требующие поиска ===
    search_phrases = [
        # Прямые запросы
        "какая погода", "какой курс", "сколько стоит",
        "когда выйдет", "когда релиз", "дата выхода", "дата релиза",
        # Игровые гайды (только с явным контекстом)
        "что собирать на", "что купить на", "что брать на",
        "билд на ", "гайд на ", "гайд по ",
        "контрпик на", "контр пик на", "контрпик против",
        "патч ноутс", "патч нот", "новый патч",
        "винрейт ", "пикрейт ",
        # Новости
        "последние новости", "что нового в",
        "кто выиграл", "кто победил",
    ]
