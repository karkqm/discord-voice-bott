...
class STTEngine:
    """Обёртка над RealtimeSTT (KoljaB/RealtimeSTT).
    ...
    def __init__(
        ...
        self._text_loop_thread: Optional[threading.Thread] = None
        self._running = False
    ...
    def _init_and_run(self) -> None:
        ...
        try:
            ...
            self._ready = True
            log.info(f"STT engine ready (model={self.model}, lang={self.language})")

            # Главный цикл — recorder.text() блокирует до получения текста
            while self._running:
                try:
            ...
            log.error(f"Failed to start STT engine: {e}")
    ...
    def _on_text(self, text: str) -> None:
        ...
        # Фильтруем мусор от STT
        # Убираем слишком короткие фразы (1-2 символа)
        if len(text) < 3:
            log.debug(f"STT filtered (too short): {text}")
            return

        # Убираем типичный мусор Whisper (галлюцинации тишины)
        garbage = {
    ...
    def feed_audio(self, audio_data: bytes, user_id: int) -> None:
        ...
        try:
            # Подаём чанками по 4096 байт (как ожидает RealtimeSTT)
            chunk_size = 4096
